...
<body>
    <div class="container">
        <h1 class="mt-5 mb-4">Crear Solicitud</h1>
            ...
            <form action="{{ route('storeSolicitud') }}" method="POST">
            ...
            <div class="form-group">
                ...
                <select class="form-control" id="servicio_id" name="servicio_id">
                    @foreach ($servicios as $servicio)
                        <option value="{{ $servicio->_id }}">{{ $servicio->nombre }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <button type="submit" class="btn btn-sm btn-primary" onclick="return confirm('¿Crear solicitud?')">
                <i class="fas fa-plus"></i> Crear Solicitud
            </button>
            
            <a href="{{ url('solicitudes/Solicitudes') }}" class="btn btn-sm btn-primary">Cancelar</a>
        </form>  
    </div>
